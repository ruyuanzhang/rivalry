...
if stereoMode == 2||stereoMode == 4 %4, you present same img with disparty, need to define disparity here
    ...
    disparity =  1 * dsdeltapx; % in pixels per eye, the binocular disparity will be twice this
    % if  1 *, that means move thing to back
    % if -1 *, that means move thing to front
    % if  0 *, that means with fixation
    
    % stupid blue rect stuff
...
% prepare window for alpha blending (br: you only need to do this once,
% move to pton)
...
elseif stereoMode == 1||stereoMode == 2||stereoMode == 3||stereoMode == 4|| stereoMode == 5
    ...
    if ~isempty(specialoverlay)
        ...
        texture = Screen('MakeTexture',win,specialoverlay);
        Screen('SelectStereoDrawBuffer', win, 0);
        Screen('DrawTexture',win,texture,[],overlayrect - [0 0 0 0],[],0);
        Screen('SelectStereoDrawBuffer', win, 1);
        Screen('DrawTexture',win,texture,[],overlayrect + [0 0 0 0],[],0);
        Screen('Close',texture);
        %%% 3D END
...
%    Screen('FillRect',win,grayval);   % REMOVED! this means do whole screen.    % ,movierect);
  ...
  % otherwise, make a texture, draw it at a particular position
    ...
    annulus_r=createMask(round(min(d1images,d2images)/2)+50,'maskType','annulus','innerRadius',round(min(d1images,d2images)/2),'background',rblumconst(1));
    annulus_b=createMask(round(min(d1images,d2images)/2)+50,'maskType','annulus','innerRadius',round(min(d1images,d2images)/2),'background',rblumconst(3));
    annulusMask_r = Screen('MakeTexture',win,annulus_r);
    annulusMask_b = Screen('MakeTexture',win,annulus_b);
    annulusRect=CenterRect([0 0 size(annulus_r,2) size(annulus_r,1)],rect) + ...
                repmat(extracircshift([2 1]),[1 2]) + ...
                [offset(1) offset(2) offset(1) offset(2)];
    %pixAddMask = a
    ...
    switch rightEyeImg
        ...
        case 3
            const(2)=rblumconst(4); %house
        case 4     
            const(2)=rblumconst(5); %car     
    end
            ...
            if size(framecolor,2) == 3  % the usual case
                ...
                elseif stereoMode == 1||2||5 %present two different images to two eyes
                    ...
                    elseif expcondorder(1,frame0) == 5
                        Screen('SelectStereoDrawBuffer', win, 0);
                        Screen('DrawTexture',win,texture,[],movierect,rotangle,filtermode,1,horzcat(rblumconst(1)/127*[255 255 255],const(1)*255));
                        Screen('DrawTexture',win,texture2,[],movierect,-rotangle,filtermode,1,horzcat(rblumconst(1)/127*[255 255 255],const(2)*255*0.5));
                        Screen('BlendFunction', win, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
                        Screen('DrawTexture',win,annulusMask_r,[],annulusRect,[],[],[],[]); % we draw a 2D round mask
                    ...
                    elseif expcondorder(1,frame0) == 6
                        Screen('SelectStereoDrawBuffer', win, 1);
                        Screen('DrawTexture',win,texture2,[],movierect,-rotangle,filtermode,1,horzcat(rblumconst(3)/127*[255 255 255],const(1)*255));
                        %Screen('BlendFunction', win, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
                        Screen('DrawTexture',win,texture,[],movierect,rotangle,filtermode,1,horzcat(rblumconst(3)/127*[255 255 255],const(2)*255*0.5));
