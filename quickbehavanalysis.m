...
load('./kk/RivalryExp.mat'); %load the experimental design file
...
%which stands for stimulus condition number in this trial
    ...
    load(['kkrun' num2str(run) '.mat']);% load behavioral data of this run
...
myplot(0:4,numel(run)*7*ones(numel(0:4),1),'--k'); hold on;
ylabel('# of Choices');
xlabel('Conditions');
set(gca,'XTick',1:4);
...
myplot(0:7,numel(run)*7*ones(numel(0:7),1),'--k'); hold on;
...
myplot(0:9,24*ones(numel(0:9),1),'--k'); hold on;
ylabel('# of Choices');
xlabel('Conditions');
ele=get(gca,'Child');
set(gca,'XTick',1:9);
